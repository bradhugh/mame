...
u64 debugger_cpu::expression_read_memory(void *param, const char *name, expression_space spacenum, u32 address, int size)
{
	switch (spacenum)
	{
		...
		case EXPSPACE_SPACE3_LOGICAL:
		{
			...
			if (device == nullptr || !device->get_interface(memory))
			{
...
expression_error::error_code debugger_cpu::expression_validate(void *param, const char *name, expression_space space)
{
	...
	case EXPSPACE_SPACE3_LOGICAL:
		...
		if (!device)
			device = get_visible_cpu();
		if (!device->get_interface(memory) || !memory->has_space(AS_PROGRAM + (space - EXPSPACE_PROGRAM_LOGICAL)))
	...
	case EXPSPACE_SPACE3_PHYSICAL:
		...
		if (!device)
			device = get_visible_cpu();
		if (!device->get_interface(memory) || !memory->has_space(AS_PROGRAM + (space - EXPSPACE_PROGRAM_PHYSICAL)))
			return expression_error::NO_SUCH_MEMORY_SPACE;
		break;
	...
	case EXPSPACE_RAMWRITE:
		...
		if (!device)
			device = get_visible_cpu();
		if (!device->get_interface(memory) || !memory->has_space(AS_PROGRAM))
			return expression_error::NO_SUCH_MEMORY_SPACE;
		break;
