...
#if defined(OSD_WINDOWS) && defined(USE_DIRECTWRITE)
...
osd_font_dwrite::osd_font_dwrite(
    ComPtr<ID2D1Factory> d2dfactory,
    ComPtr<IDWriteFactory> dwriteFactory,
    ComPtr<IWICImagingFactory> wicFactory)
{
    // Assign our factories that were passed in
...
bool osd_font_dwrite::open(const char *font_path, const char *_name, int &height)
{
    ...
    // Since the GDI implementation actually provided cell height for LOGFONT::lfheight (a positive value)
    // Value: > 0 "The font mapper transforms this value into device units and matches it against the cell height of the available fonts."
    // we need to figure out the EM height from the cell height
    // The below formula seems like it should work but doesn't
    //DEFAULT_FONT_HEIGHT - (((4.0f * ((3.0f * DEFAULT_FONT_HEIGHT / (4.0f * metrics.designUnitsPerEm)) - ((float)(metrics.ascent - metrics.descent) / pow(2, metrics.designUnitsPerEm)))) / 3.0) * DEFAULT_FONT_HEIGHT);
...
HRESULT osd_font_dwrite::find_font(
...
bool osd_font_dwrite::get_bitmap(unicode_char chnum, bitmap_argb32 &bitmap, INT32 &width, INT32 &xoffs, INT32 &yoffs)
{
...
public:
    ...
    virtual int init(const osd_options &options) override
    {
        ...
        HR_RET1(D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, this->m_d2dfactory.GetAddressOf()));
...
void SaveBitmap(IWICBitmap* bitmap, GUID pixelFormat, const WCHAR *filename)
{
    ...
    // Create a Direct2D factory
    HR_RET(D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, d2dfactory.GetAddressOf()));

    // Initialize COM - ignore failure
    CoInitializeEx(NULL, COINIT_APARTMENTTHREADED);

    // Create a DirectWrite factory.
    HR_RET(DWriteCreateFactory(
        DWRITE_FACTORY_TYPE_SHARED,
        __uuidof(IDWriteFactory),
...
#else /* Any other platform than windows, or if MODERN_WIN_API is not defined */
